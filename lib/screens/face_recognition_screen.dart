...
class _FaceRecognitionScreenState extends State<FaceRecognitionScreen> {
  ...
  LatLng _officeLocation = LatLng(-7.638569386167681, 111.53899216716499); // Lokasi kantor
  ...
  Future<void> _submitAttendance(BuildContext context) async {
    ...
    if (profileResponse.statusCode == 200) {
      ...
      final body = {
        ...
        'jam_masuk': widget.action == 'Clock In' ? formattedTime : null,
        'jam_keluar': widget.action == 'Clock Out' ? formattedTime : null,
        'foto_masuk': widget.action == 'Clock In' ? await getFileAsBase64(widget.pickedImage?.path) : null,
        'foto_keluar': widget.action == 'Clock Out' ? await getFileAsBase64(widget.pickedImage?.path) : null,
        'latitude_masuk': widget.action == 'Clock In' ? latitude : null,
        'longitude_masuk': widget.action == 'Clock In' ? longitude : null,
        'latitude_keluar': widget.action == 'Clock Out' ? latitude : null,
        'longitude_keluar': widget.action == 'Clock Out' ? longitude : null,
        'lokasi_masuk': widget.action == 'Clock In' ? 'Lokasi Dinamis' : null,
        'lokasi_keluar': widget.action == 'Clock Out' ? 'Lokasi Dinamis' : null,
      };
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 20),
              Text(
                widget.action == 'Clock In' ? 'Jam Masuk: ${widget.time}' : 'Jam Keluar: ${widget.time}',
